...
const expectedResult = getFixturePath('expectedResult.yml');
const nestedPath1 = getFixturePath('nestedFile1.json');
const nestedPath2 = getFixturePath('nestedFile2.json');
const nestedResult = getFixturePath('nestedResult.yml');

test('Stylish generation', () => {
  expect(generateDiff(jsonPath1, jsonPath2)).toEqual(readFileSync(expectedResult, 'utf-8'));
  expect(generateDiff(yamlPath, ymlPath)).toEqual(readFileSync(expectedResult, 'utf-8'));
  expect(generateDiff(jsonPath1, ymlPath)).toEqual(readFileSync(expectedResult, 'utf-8'));
